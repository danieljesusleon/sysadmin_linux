...
function menuquestion {
	ans=$(zenity  --list  --title="Seleccione que desea respaldar" --text "     Seleccione una opción" --radiolist  --column "Pick" --column "Opicion" TRUE Archivo FALSE Directorio FALSE Salir); echo $ans ; echo $
	if [ $? = 0 ];then
		exit
	else
...
#}
function inputfile {
	FILE=`zenity --file-selection --title="Seleccione el $ans a respaldar"` 2>/dev/null
...
#	 tar -cvf FACMES-`date +%F`.tar filtrado.txt
